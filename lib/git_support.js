...
module.exports = git = {
  getConfigItem: function(superTask, repo, configItem, callback) {
    ...
    try {
      return Shell.spawn(superTask, 'git', ["--git-dir=" + gitDir, 'config', '--get', configItem], function(error, result) {
        if (error) {
  ...
  getStatus: function(superTask, repo, callback) {
    ...
    gitDir = "" + repo.workDir + "/.git";
    return Shell.spawn(superTask, 'git', ["--git-dir=" + gitDir, "--work-tree=" + repo.workDir, 'status'], function(error, status) {
      if (error) {
        return callback(error);
      }
