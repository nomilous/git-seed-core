...
module.exports = git = {
  ...
  getStatus: function(repo, callback) {
    var gitDir;
  ...
  missingRepo: function(repo, callback) {
    var gitDir, missing;
    gitDir = "" + repo.workDir + "/.git";
    missing = false;
    if (!Shell.gotDirectory(gitDir)) {
  ...
  wrongBranch: function(repo, callback) {
    repo.status || (repo.status = {});
    if (repo.status['missing repo']) {
      return callback(null, repo);
    }
  ...
  status: function(superTask, repo, args, callback) {
    ...
    return pipeline([
      function() {
        return nodefn.call(git.missingRepo, input);
      }, function(assemble) {
        return nodefn.call(git.wrongBranch, assemble);
      }, function(assemble) {
        return nodefn.call(git.getStatus, assemble);
      }
  ...
  clone: function(superTask, workDir, origin, branch, callback) {
    var cloneArgs;
    ...
    return sequence([
      function() {
        ...
        return nodefn.call(Shell.spawn, superTask, 'git', ['clone', origin, workDir]);
      }, function() {
        return nodefn.call(Shell.spawn, superTask, 'git', git.checkoutArgs(workDir, branch));
      }
